...
<c:set var="activePage" value="register-otp" scope="request"/>
<body>
    ...
    <div class="container">
        <div class="center">
            <div class="card">
                ...
                <c:if test="${not empty sessionScope.otpErrorMessage}">
                    <div class="error" id="errorBox">${sessionScope.otpErrorMessage}</div>
                    <c:remove var="otpErrorMessage" scope="session"/>
                </c:if>
                ...
                <form method="post" action="${pageContext.request.contextPath}/register/verify-otp" id="otpForm" novalidate>
                    <input type="hidden" name="username" value="${sessionScope.regUsername}">
                    <input type="hidden" name="email" value="${sessionScope.regEmail}">
                    <input type="hidden" name="password" value="${sessionScope.regPassword}">
                    <input type="hidden" name="confirmPassword" value="${sessionScope.regPassword}">
                    <div class="field">
...
<script>
    ...
    function resendOtp() {
        ...
        let confirmPassword = confirmPasswordEl ? confirmPasswordEl.value : '';

        if (!username) username = '${sessionScope.regUsername}';
        if (!email) email = '${sessionScope.regEmail}';
        if (!password) password = '${sessionScope.regPassword}';
        if (!confirmPassword) confirmPassword = '${sessionScope.regPassword}';
        ...
        addInput('confirmPassword', confirmPassword);
